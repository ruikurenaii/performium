/*
  ...
  011726.ts: the pp system used for v1.6.1.
  This only adds a few things for other files.
  Prior to this system, notes are the only file types that give additional performance values.
...
export async function calculatePerformance(plugin: PerformiumPlugin): Promise<number> {
  ...
  let accuracyValue = angleValue > 0 ? ((await angleValue / 360) * 100 + (1 / (angleValue / 360))) * ACCURACY_MULTIPLIER : 0;
