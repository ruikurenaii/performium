/*
  ...
  050725.ts: the newer version of the pp system.
  this change includes an "almost complete" rewrite of the system, adding various reworks similar to changes made to the osu!pp system.
  thanks to @mnaoumov in discord for helping me fix the problem using the totalFocusTime setting value for this pp system.
...
export async function calculatePerformance(plugin: PerformiumPlugin): Promise<number> {
  ...
  let performanceValue: number = ((angleBonus + starRatingBonus) / 2.05) + (combinedValue * (await starRating / 2.3)) + (roughnessPenalty * (3.1415926535 / 0.875)) + (factorBonus / (2.7182818284 * 1.05)) + (overallPenalty / -1.1) + (timeBonus / 9.8) + bonusValue;
