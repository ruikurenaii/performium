/*
  ...
  060925.ts: the version of the pp system used for performium v1.5.0.
...
export async function calculatePerformance(plugin: PerformiumPlugin): Promise<number> {
  ...
  let accuracyValue = (await angleValue / 360) * 100;
  let speedValue = overallTime > 0 ? totalWords / overallTime : 0;
  ...
  // apply the file count bonus to the system
