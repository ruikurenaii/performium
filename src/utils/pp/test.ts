...
export async function calculatePerformance(plugin: PerformiumPlugin): Promise<number> {
  ...
  const sliderComplexityMultiplier = 1 + ((0.2 * Math.log2(1 + averageWordsPerSentence) + 0.2 * Math.log2(1 + averageSentenceLength) + 0.15 * Math.log2(1 + longestSentenceLength) + 0.15 * Math.log2(1 + longestParagraphLength) + 0.1 * Math.log2(1 + totalTags) + 0.2 * Math.log2(1 + vaultObjects.sliders)) / 2.25);
