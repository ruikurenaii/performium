/*
  ...
  013026.ts: the second cancelled version of the pp system.
  this is due to the unrelativity of the calculation of performance points.
  
*/

/*

  for v1.7.0, the update will focus on a complete rewrite of the performance system.
  for this one, I have remvoed certain reworks that were on the previous system (v1.6.1)
...
export async function calculatePerformance(plugin: PerformiumPlugin): Promise<number> {
  ...
  async function calculateStrain() {
    ...
    */
    ...
    let totalCharactersPerWord = totalWords / totalChars;
    let totalWordsPerCharacter = totalChars / totalWords;

    strainValue += totalCharactersPerWord * (1 + (totalWordsPerCharacter));
