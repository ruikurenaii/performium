/*
  ...
  021326.ts: the unused test version of the pp system.
...
export async function calculatePerformance(plugin: PerformiumPlugin): Promise<number> {
  ...
  // also scale aim pp with the acute angle bonus
  aimValue *= 1 + 0.25 * (finalAngleValue / (152.5 ** (finalAngleValue * 0.00075)));

  // add tag bonus to speed pp
  speedValue += Math.log10(totalTags);

  // multiply the strain value by the amount of all files divided by a high number
  strainValue *= 1 + (totalFiles / 25000);

  // conditional statements
  if (averageCharsPerSentence > 50) {
    strainValue += Math.log(averageCharsPerSentence) + Math.max(0, Math.log(averageCharsPerSentence - 49));
  } else {
    strainValue += Math.log(averageCharsPerSentence);
  }
  ...
  // scale aim and accuracy pp (no game modifiers support since they're not possible as an addition to the system.)
  aimValue *= 1.08;
  accuracyValue *= 1.08;
